        ...
        <div class="main">
            ...
            <div class="report-container">
                ...
                <div class="report-header">
                    ...
                    <a class="t-op-nextlvl" style="background-color: rgb(60, 229, 255); padding:10px; border-radius:14px;"
                        href="{{ route('dashbordlivre.create') }}" class="btn btn-primary float-right">Add a book</a>

                </div>

                <div class="report-body" style="height: 600px; overflow-y: auto;">
                    <div class="report-topic-heading">
                        <h3 class="t-op">ID</h3>
                        <h3 class="t-op">IMAGE</h3>
                        <h3 class="t-op">ISBN</h3>
                        <h3 class="t-op">TITLE</h3>
                        <h3 class="t-op">DESCRIPTION</h3>
                        <h3 class="t-op">PRICE</h3>
                        <h3 class="t-op" colspan="2">ACTIONS</h3>
                    </div>

                    <div class="items">
                        @forelse ($livres as $livre)
                            <div class="item1">
                                <h3 class="t-op-nextlvl">{{ $livre->id }}</h3>
                                <h3 class="t-op-nextlvl"><img src="{{ $livre->image_Livre }}" width="80px" height="100px"
                                        alt=""></h3>
                                <h3 class="t-op-nextlvl">{{ $livre->ISBN_Livre }}</h3>
                                <h3 class="t-op-nextlvl">{{ Str::limit($livre->titre_Livre, 10, '...') }}</h3>
                                <h3 class="t-op-nextlvl">{{ Str::limit($livre->description_Livre, 10, '...') }}</h3>
                                <h3 class="t-op-nextlvl">{{ $livre->prix_Livre }}</h3>
                                <div class="t-op-nextlvl actions" style="  grid-column: span 2;">
                                    <h3 class="t-op-nextlvl label-tag"><a class="action-link"
                                            href="{{ route('dashbordlivre.edit', $livre->id) }}">Edit</a>
                                    </h3>
                                    <form id="deleteForm{{ $livre->id }}"
                                        action="{{ route('dashbordlivre.destroy', $livre->id) }}" method="POST"
                        ...
                        @empty
                            <div class="item1">
                                <h3 class="t-op-nextlvl">No books available</h3>
                            </div>
                        @endforelse
                    </div>
                </div>
